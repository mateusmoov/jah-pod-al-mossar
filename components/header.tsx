  ...
  return (
        ...
        <motion.div
         className="fixed top-0 left-1/2 h-[4.5rem] w-full rounded-none border border-white border-opacity-40
          bg-gray-900 bg-opacity-80 shadow-lg shadow-black/[0.03] backdrop-blur-[0.5rem]
         ...
         <nav className="flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
           <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem]
             text-[#f3f3f3]  font-bold sm:w-[initial] sm:flex-nowrap sm:gap-5">
            {links.map((link) => (
                <motion.li 
                ...
                >
                 <Link
                  className={clsx("flex w-full items-center justify-center px-3 py-3 hover:text-[#782BF1] transition",
                  {
                    "text-[#782BF1]": activeSection === link.name,
                  }
                    )}
                  href={link.hash}
                  onClick={()=> {
